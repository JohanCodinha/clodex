...
| `cc260-patch10-execute-real-builder` | The same execution proof against **all eight** real 2.1.260 bundles, the release whose builder stopped reading the remote flag through `process.env` (`i=a.CLAUDE_CODE_REMOTE===!0` off a module-level snapshot), so the head anchor spelling the call form refused every platform. Recovers 28 free identifiers from the builder's own text and drives both the early-return and the full credential-scrub path. Copy this one when a new release moves the builder again. |
